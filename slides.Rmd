...
Then open `lplus-2026-dataprocessing-ws/lplus-2026-dataprocessing-ws.Rproj`.
...
```{r eval=FALSE}
filter(chinese_ldt, is.na(rt)) # find missing values
```

```{r eval=FALSE}
filter(chinese_ldt, !is.na(rt)) # remove missing values
```
...
- Reshaping data frame orientation using `pivot_wider` / `pivot_longer` 
